...
tf.flags.DEFINE_bool("pretrained_embeddings", False, 'Add pretraiend embedding (change embedding dim fscoreordingly)')
...
def train(data):
    ...
    for epoch in range(FLAGS.epochs):
        # Training step
        step, train_loss, train_fscore, train_fscore_macro = train_step(cnn_x_train, cnn_y_train, 
            gcn_features, gcn_support, gcn_y_train, gcn_train_mask, gcn_placeholders)
        
        # Validation step
        step, val_loss, val_fscore, val_fscore_macro = evaluate(cnn_x_dev, cnn_y_dev, 
...
def main(argv=None):
    ...
    log.info("Training and Testing model TEXT GCN CNN--------")
    ...
    log.info("sigmoid threshold: {}".format(FLAGS.sigmoid_threshold))
