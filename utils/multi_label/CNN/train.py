...
tf.flags.DEFINE_bool("umls_embedding", False, "Add pretrained umls embeddings.")
...
def train(x_train, y_train, vocab_processor, x_dev, y_dev, x_test, y_test ):
    ...
    if FLAGS.umls_embedding:
        ...
        initW_umls = data_helpers.get_umls_embedding(vocab_processor, initW_umls, FLAGS.umls_embedding_filename)
    ...
    def train_step(x_batch, y_batch):
        ...
        # append to summary
        fscore_micro = metrics.f1_score(y_true = y_batch, y_pred = predictions, average= 'micro')
        fscore_macro = metrics.f1_score(y_true = y_batch, y_pred = predictions, average= 'macro')
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        log.debug("step {} \t train_loss {:g} \t train_acc {:g} \t dev_loss {:g} \t dev_fscore {:g}".format(step, train_loss, train_acc, dev_loss, dev_fscore))
