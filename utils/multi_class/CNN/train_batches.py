...
def train(x_train, y_train, vocab_processor, x_dev, y_dev, x_test, y_test ):
    ...
    # Summaries for loss and accdev_acc_micro
    ...
    for batch in batches:
        ...
        log.debug("step {} \t currstep {} \t train_loss {:g} \t train_acc {:g}".format(step, current_step, train_loss, train_acc))

        # evaluate every epocch = every numb batches
        if current_step % FLAGS.batch_size == 0:
            ...
            if dev_loss_list[-1] > np.mean(dev_loss_list[-(FLAGS.batch_size+1):-1]):
                ...
                break
            # if the validation dev_acc doesn't increase in 20 current_steps early stop
            if val_acc_list[-1] <= np.mean(val_acc_list[-(FLAGS.batch_size+1):-1]) and current_step > 20:
                log.info("Early stopping (same accdev_acc), current_step: {}".format(current_step))
                break

            log.debug("step {} \t dev_loss {:g} \t dev_acc {:g}".format(current_step, dev_loss, dev_acc))

        
def main(argv=None):
        
    # Logging 
    log.basicConfig(filename= os.path.join(os.path.curdir ,'experiments_cnn_batches.log'), level=log.DEBUG)
