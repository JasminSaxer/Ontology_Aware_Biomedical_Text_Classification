...
tf.flags.DEFINE_float('softmax_threshold', 0.5, 'Threshold for softmax (>= threshold)')
...
def train(x_train, y_train, vocab_processor, x_dev, y_dev, x_test, y_test ):
    ...
    # if pretrained ont emebds
    np.random.seed(42)
    ont_embedding_dim = 50
    init_ont_W = np.random.uniform(-1,1,(len(vocab_processor.vocabulary_), ont_embedding_dim))
    init_ont_W = data_helpers.get_umls_embedding(vocab_processor, init_ont_W)
    init_ont_W = np.float32(init_ont_W)
    ...
    cnn = TextCNN(
        ...
        softmax_threshold = FLAGS.softmax_threshold,
        ont_embeds = init_ont_W,
        bioasq_embed = initW)
    ...
    # change to training loop for each epoch (only one batch)
    dev_loss_list = []
    max_dev_acc = 0
    for epoch in range(FLAGS.num_epochs):

        step, train_loss, train_acc = train_step(x_train, y_train)
        dev_loss, dev_acc = dev_step(x_dev, y_dev, writer=dev_summary_writer)
        dev_loss_list.append(dev_loss)
        
        log.debug("step {} \t train_loss {:g} \t train_acc {:g} \t dev_loss {:g} \t dev_acc {:g}".format(step, train_loss, train_acc, dev_loss, dev_acc))

        if epoch > 0:
            if dev_acc >= max_dev_acc and dev_loss <= dev_loss_list[-2]:
                max_dev_acc = dev_acc
                path = saver.save(sess, checkpoint_prefix, global_step=epoch)
                log.info("Saved model checkpoint to {}".format(path))
            
        # if the current loss is higher than the mean of the last 10 epochs validation losses -> early stopping
        if dev_loss_list[-1] > np.mean(dev_loss_list[-(10+1):-1]):
            log.info("Early stopping at epoch {}".format(step))
            break
...
def main(argv=None):
    ...
    # Logging 
    log.basicConfig(filename= os.path.join(os.path.curdir ,'experiment_cnn.log'), level=log.DEBUG)
    log.info("Experiment : {} ------------------------------------------------------------------".format(FLAGS.exp_name))
